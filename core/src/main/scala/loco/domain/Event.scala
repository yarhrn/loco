...
case class MetaAggregateBuilder[E <: Event, A <: Aggregate[E]](aggregateBuilder: AggregateBuilder[A, E]) {
  def empty(aggregateId: AggregateId[E]) =
    MetaAggregate[E, A](aggregateBuilder.empty(aggregateId), AggregateVersion(0))
...
object MetaEvent {
  def fromRawEvents[E <: Event](
    ...
    val versions =
      (1 to events.size).map(counter => AggregateVersion[E](lastKnownVersion.version + counter))
